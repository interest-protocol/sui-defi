...
module clamm::tick_bit_map {
  ...
  use i256::i256::{shr, mod, I256, as_u256, from, div, is_neg, is_zero, sub, one, mul, add};
  ...
  public fun position(tick: &I256): (I256, u8) {
    (shr(tick, 8), (as_u256(&mod(tick, &from(256))) as u8))
  }
  ...
  public fun flip_tick(bit_map: &mut Table<I256, State>, tick: &I256, tick_spacing: &I256) {
    ...
    let mask: u256 = 1 << bit;
    let state = table::borrow_mut(bit_map, word);
  ...
  public fun next_initialized_tick_within_one_word(
    ...
    if (lte) {
      ...
      if (masked != 0) {
          (mul(&sub(&compressed, &from((bit - most_significant_bit(state.bits) as u256))), tick_spacing), true)
      } else {
      ...
      if (masked != 0) {
         (mul(&add(&add(&compressed, &one()), &from(((least_significant_bit(state.bits) - bit) as u256))), tick_spacing), true)
      } else {
