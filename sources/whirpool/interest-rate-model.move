module interest_protocol::interest_rate_model {
  ...
  use interest_protocol::math::{d_fdiv, d_fmul, double_scalar};
  ...
  struct InterestRateData has key, store {
    id: UID,
    base_rate_per_ms: u64,
    multiplier_per_ms: u64,
    jump_multiplier_per_ms: u64,
    kink: u64
  }
  ...
  public fun get_supply_rate_per_epoch(
    ...
    let borrow_rate = d_fmul(get_borrow_rate_per_epoch_internal(storage, market_key, cash, total_borrow_amount, reserves), (double_scalar() as u64) - reserve_factor);

    d_fmul(get_utilization_rate_internal(cash, total_borrow_amount, reserves), borrow_rate)
  }
  ...
  fun get_borrow_rate_per_epoch_internal(
    ...
    reserves: u64
      ...
      if (data.kink >= utilization_rate) {
        d_fmul(utilization_rate, data.multiplier_per_ms) + data.base_rate_per_ms
      } else {
        let normal_rate = d_fmul(data.kink, data.multiplier_per_ms) + data.base_rate_per_ms;

        let excess_utilization = utilization_rate - data.kink;
        
        d_fmul(excess_utilization, data.jump_multiplier_per_ms) + normal_rate
      }
    }
  ...
  public(friend) fun set_interest_rate_data<T>(
    ...
    let ms_per_year = get_ms_per_year();
